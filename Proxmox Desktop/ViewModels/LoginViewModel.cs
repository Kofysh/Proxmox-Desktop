...
public partial class LoginViewModel : ObservableObject
{
    ...
    public LoginViewModel(ConfigurationService config) { _config = config; LoadSaved(); }
    ...
    public async Task LoadRealmsAsync()
    {
        ...
        try { _api = new ApiClient(Server.Trim(), Port.Trim(), SkipSsl); foreach (var r in await _api.GetRealmsAsync()) Realms.Add(r); SelectedRealm = Realms.FirstOrDefault(); }
    ...
    public async Task LoginAsync()
    {
        ...
        try
        {
            var result = UseApiToken
                ? await _api.LoginWithTokenAsync(TokenId.Trim(), TokenSecret.Trim())
                : await _api.LoginAsync(Username.Trim(), Password, SelectedRealm?.Realm ?? "pam", TotpVisible ? Otp : null);
    ...
    private bool CanLogin()
    {
        ...
        return UseApiToken
            ...
            : !string.IsNullOrWhiteSpace(Username) && !string.IsNullOrWhiteSpace(Password) && SelectedRealm is not null;
    }
    ...
    {
        _config.Set("server", Server); _config.Set("port", Port); _config.Set("skipSsl", SkipSsl); _config.Set("useApiToken", UseApiToken);
        if (UseApiToken) _config.Set("tokenId", TokenId); else _config.Set("username", Username);
        _config.Save();
    }
}
